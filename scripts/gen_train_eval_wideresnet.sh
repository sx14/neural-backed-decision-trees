...
for i in "CIFAR10 ${MODEL_NAME}_cifar10 1"; do
  read dataset model weight <<< "${i}";

  # 1. generate hieararchy
  nbdt-hierarchy  --dataset=${dataset} --arch=${model} --method=induced2

  # 2. train with soft tree supervision loss
  python main.py --lr=0.01 --dataset=${dataset} --arch=${model} --hierarchy=induced2-${model} --pretrained --loss=SoftTreeSupLoss --tree-supervision-weight=${weight}
  ...
  for analysis in SoftEmbeddedDecisionRules HardEmbeddedDecisionRules; do
    python main.py --dataset=${dataset} --arch=${model} --hierarchy=induced2-${model} --loss=SoftTreeSupLoss --eval --resume --analysis=${analysis} --tree-supervision-weight=${weight}
